...
public class CODEGEN_Utils {
	...
	public static CODEGEN_Temporary codeGen_malloc(CODEGEN_StringNLBuilder printed,int allocationSize) throws IOException, SEMANTIC_TempsPastLimitException{
		printed.appendNL(String.format("li $a0,%d",allocationSize));
		return common_codeGen_malloc(printed);
	}
	
	public static CODEGEN_Temporary codeGen_malloc(CODEGEN_StringNLBuilder printed,CODEGEN_Temporary allocationSize) throws IOException, SEMANTIC_TempsPastLimitException{
		printed.appendNL(String.format("mov $a0,%s",allocationSize.getName()));
		return common_codeGen_malloc(printed);

	}
	
	private static CODEGEN_Temporary common_codeGen_malloc(CODEGEN_StringNLBuilder printed) throws IOException, SEMANTIC_TempsPastLimitException{
